...
public:
    inline ThreadPoolExecutor(unsigned short size = 4) { addThread(size); }
            ...
            _pool.emplace_back( [this]{ //工作线程函数
                while (_run)
                {
                    Task task; // 获取一个待执行的 task
                    {
                        ...
                        _task_cvl.wait(lock, [this]{
                            return !_run || !_tasks.empty();
                        }); // wait 直到有 task
                        //run all the tasks cached in the task queue even threadpool getting the stop command
